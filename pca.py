...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run PCA on trained P(bind) model feature space."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory containing predictions.csv and config.json. "
            "Example: models/PXR_full"
        ),
    )
    ...
    parser.add_argument(
        "--predictions_csv",
        default=None,
        help="Path to predictions.csv. Overrides --model_dir if provided.",
    )

    parser.add_argument(
        "--config_json",
        default=None,
        help="Path to config.json. Overrides --model_dir if provided.",
    )

    parser.add_argument(
        "--output_dir",
        default=None,
        help="Directory to save PCA outputs. Default: <model_dir>/pca_outputs",
    )

    args = parser.parse_args()

    if args.model_dir is None and (
        args.predictions_csv is None or args.config_json is None
    ):
        raise ValueError(
            "Provide either --model_dir or both --predictions_csv and --config_json."
        )

    if args.model_dir is not None:
        model_dir = Path(args.model_dir)
        predictions_csv = Path(args.predictions_csv) if args.predictions_csv else model_dir / "predictions.csv"
        config_json = Path(args.config_json) if args.config_json else model_dir / "config.json"
        output_dir = Path(args.output_dir) if args.output_dir else model_dir / "pca_outputs"
    else:
        predictions_csv = Path(args.predictions_csv)
        config_json = Path(args.config_json)
        output_dir = Path(args.output_dir) if args.output_dir else Path("pca_outputs")

    if not predictions_csv.exists():
        raise FileNotFoundError(f"Could not find predictions CSV: {predictions_csv}")

    if not config_json.exists():
        raise FileNotFoundError(f"Could not find config JSON: {config_json}")
